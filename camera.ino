...
String runInference(camera_fb_t* fb) {
    ...
    // Only wrap in array if there are detections
    if (!firstDetection) {
...
void sendData(const uint8_t* imageData, size_t imageLen, const char* prediction) {
    ...
    // Create buffer for full payload
    size_t bodyStartLen = body.length();
    size_t totalLen = bodyStartLen + imageLen + 2; // +2 for \r\n
    ...
    // Construct payload
    memcpy(postData, body.c_str(), bodyStartLen);
    memcpy(postData + bodyStartLen, imageData, imageLen);
    memcpy(postData + bodyStartLen + imageLen, detectionPart.c_str(), detectionPart.length());
...
void loop() {
    ...
    // Only send data if inference found objects (jsonResult is not "[]")
    if (jsonResult != "[]") {
        Serial.println("Found objects - sending to server");
        sendData(fb->buf, fb->len, jsonResult.c_str());
    } else {
        Serial.println("No objects detected - skipping server upload");
    }
